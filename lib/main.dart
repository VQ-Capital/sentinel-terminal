...
class _DashboardScreenState extends ConsumerState<DashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Latency hesaplamaları (Sadece son 100 işlem)
              ...
              builder: (context, constraints) {
                ...
                if (isDesktop) {
                  return Padding(
                    ...
                    child: Column(
                      children: [
                        SizedBox(
                            height: 100,
                            child: _buildStatsRow(displayBalance, displayRealizedPnL, displayUnrealizedPnL, winRate, avgLatency)),
                  ...
                  return ListView(
                    ...
                    children: [
                      _buildStatsGrid(constraints.maxWidth, displayBalance, displayRealizedPnL, displayUnrealizedPnL, winRate, avgLatency),
                      const SizedBox(height: 16),
                      SizedBox(height: 250, child: _buildPnLChartContainer("KÜMÜLATİF PnL", PnLChartPainter(pnlHistory), displayRealizedPnL)),
                      const SizedBox(height: 16),
                      SizedBox(height: 200, child: _buildLatencyChartContainer("SLA WATCHDOG (LATENCY)", LatencyChartPainter(recentLatencies), avgLatency)),
                      const SizedBox(height: 16),
                  ...
                  child: Center(
                    child: RichText(
                      text: TextSpan(
                        children: [
                          TextSpan(
                              text: "${symbol.replaceAll('usdt', '').replaceAll('USDT', '').toUpperCase()} ",
                              style: const TextStyle(color: Colors.white54, fontWeight: FontWeight.bold, fontSize: 13)),
                          TextSpan(
                              text: price.toStringAsFixed(2),
                              style: const TextStyle(color: Colors.greenAccent, fontFamily: 'monospace', fontWeight: FontWeight.bold, fontSize: 14)),
                        ],
                      ),
                    ),
                  ),
        ...
        child: Row(
          ...
          children: [
            ...
            Text(isDefensive ? "DEFANS M." : "AKTİF",
                style: TextStyle(color: isDefensive ? Colors.orangeAccent : Colors.blueAccent, fontSize: 10, fontWeight: FontWeight.bold)),
          ],
        ),
      ),
    );
  ...
  Widget _buildStatsRow(double bal, double rpnl, double upnl, double winRate, int avgLatency) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        ...
        const SizedBox(width: 12),
        Expanded(child: _buildStatCard("WIN RATE", "%${winRate.toStringAsFixed(1)}", winRate >= 50 ? Colors.blueAccent : Colors.orangeAccent, "Başarı Oranı")),
        const SizedBox(width: 12),
        Expanded(child: _buildStatCard("AĞ GECİKMESİ", "${avgLatency}ms", avgLatency > 50 ? Colors.redAccent : Colors.greenAccent, "Ortalama SLA (Ping)")),
        const SizedBox(width: 12),
  ...
  Widget _buildStatsGrid(double width, double bal, double rpnl, double upnl, double winRate, int avgLatency) {
    return GridView.count(
      ...
      children: [
        ...
        _buildStatCard("AĞ PING", "${avgLatency}ms", avgLatency > 50 ? Colors.redAccent : Colors.greenAccent, "SLA Limit: 50ms"),
        _buildKillSwitch(),
      ],
    );
  }
  ...
  Widget _buildPnLChartContainer(String title, CustomPainter painter, double currentPnl) {
    return Container(
      width: double.infinity,
  ...
  Widget _buildLatencyChartContainer(String title, CustomPainter painter, int avgLatency) {
        ...
        children: [
          Row(
            ...
            children: [
              Text(title, style: const TextStyle(color: Colors.white54, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1)),
              Text("${avgLatency}ms",
                  style: TextStyle(color: latColor, fontSize: 16, fontWeight: FontWeight.w900, fontFamily: 'monospace')),
            ],
          ),
          const SizedBox(height: 20),
          Expanded(child: ClipRect(child: CustomPaint(painter: painter, size: Size.infinite))),
...
class PnLChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final gradient = LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [paint.color.withOpacity(0.3), paint.color.withOpacity(0.0)]);
...
class LatencyChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 50ms Kırmızı SLA İhlal Çizgisi
    final slaPaint = Paint()
      ..color = Colors.redAccent.withOpacity(0.5)
      ..strokeWidth = 1
      ..style = PaintingStyle.stroke;

    int maxLat = latencies.reduce(max);
    if (maxLat < 60) maxLat = 60; // Skalayı çok daraltmamak için minimum tavan

    final double slaY = size.height - ((50 / maxLat) * size.height);
    _drawDashedLine(canvas, Offset(0, slaY), Offset(size.width, slaY), slaPaint);

    final barWidth = (size.width / latencies.length) - 2;
    final actualBarWidth = barWidth > 1 ? barWidth : 1.0;

    for (int i = 0; i < latencies.length; i++) {
      final lat = latencies[i];
      final x = i * (size.width / latencies.length);
      final normalizedY = lat / maxLat;
      final y = size.height - (normalizedY * size.height);

      Color barColor = Colors.greenAccent; // Ultra hızlı
      if (lat > 50) barColor = Colors.redAccent; // SLA İhlali (Ağ Yavaş)
      else if (lat > 20) barColor = Colors.blueAccent; // Normal Binance hızı
  ...
  @override
  bool shouldRepaint(covariant LatencyChartPainter oldDelegate) => true;
}