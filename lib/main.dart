...
class _DashboardScreenState extends ConsumerState<DashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final equityData = ref.watch(equityProvider); // Wallet'tan gelen veriler

    const double initialBalance = 10.00;

    // --- 1. LOKAL HESAPLAMALAR (Grafik ve Geçmiş Çizimi İçin) ---
    ...
    for (var r in reports.reversed) {
      ...
      bool isClosing = (r.side == "SELL" && posQty > 0.0) ||
          (r.side == "BUY" && posQty < 0.0);
      ...
      if (r.side == "SELL" && posQty > 0.0) {
        ...
        double newQty =
            r.side == "BUY" ? posQty + r.quantity : posQty - r.quantity;
        double totalValue = (posQty.abs() * avgPrice) +
            (r.quantity * r.executionPrice);
    ...
    // Yüzen PnL'in lokal yedeği (Wallet kapalıysa kullanılır)
    ...
    double winRate =
        closedTrades > 0 ? (winningTrades / closedTrades) * 100 : 0.0;

    // --- 2. SOURCE OF TRUTH (Wallet verisi lokal veriyi ezer) ---
    final displayBalance = equityData?.totalEquityUsd ?? (initialBalance + localRealizedPnL);
    final displayRealizedPnL = displayBalance - initialBalance;
    final displayUnrealizedPnL = equityData?.totalUnrealizedPnl ?? localUnrealizedPnL;
    
    final bool isDefensiveMode = ((initialBalance - displayBalance) / initialBalance) > 0.15;
                ...
                if (isDesktop) {
                            ...
                            children: [
                              Expanded(
                                  ...
                                  child: _buildChartContainer(
                                      "KÜMÜLATİF PnL",
                                      PnLChartPainter(pnlHistory),
                                      displayRealizedPnL)),
                              const SizedBox(width: 16),
                              Expanded(
                                  flex: 3,
                                  child: _buildOpenPositionsPanel(
                                      positions, avgPrices, marketPrices)),
                              const SizedBox(width: 16),
                              Expanded(
                                  flex: 4,
                                  child: _buildDenseTradeLog(reports, true)),
                            ],
                          ),
                        ),
                      ],
                  ...
                  return ListView(
                    ...
                    children: [
                      ...
                      SizedBox(
                          height: 500,
                          child: _buildDenseTradeLog(reports, false)),
                    ],
                  );
                }
              },
  ...
  Widget _buildMarketTickerTape(Map<String, double> marketPrices) {
    return Container(
      ...
      child: marketPrices.isEmpty
          ? const Center(
              child: Text("Borsa Veri Akışı Bekleniyor...",
                    ...
                    child: RichText(
                      text: TextSpan(
                        children: [
                          TextSpan(
                              text:
                                  "${symbol.replaceAll('usdt', '').replaceAll('USDT', '').toUpperCase()} ",
        ...
        child: Row(
          ...
          children: [
            ...
            Text(isDefensive ? "DEFANS M." : "AKTİF",
                style: TextStyle(
                    color:
                        isDefensive ? Colors.orangeAccent : Colors.blueAccent,
                    fontSize: 10,
                    fontWeight: FontWeight.bold)),
          ],
        ),
  ...
  Widget _buildStatsRow(
      double bal, double rpnl, double upnl, double winRate) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Expanded(
            child: _buildStatCard("CÜZDAN (NET)",
                "\$${bal.toStringAsFixed(3)}", Colors.white, "Sermaye + Kâr")),
        const SizedBox(width: 12),
        Expanded(
            child: _buildStatCard(
                "KAPANAN PnL",
                "\$${rpnl.toStringAsFixed(3)}",
                rpnl >= 0 ? Colors.greenAccent : Colors.redAccent,
                "Realize Edilen")),
        const SizedBox(width: 12),
        Expanded(
            child: _buildStatCard(
                "AÇIK PnL",
                "\$${upnl.toStringAsFixed(3)}",
                upnl >= 0 ? Colors.greenAccent : Colors.redAccent,
                "Yüzen (Floating)")),
        const SizedBox(width: 12),
        Expanded(
            child: _buildStatCard(
                ...
                winRate >= 50 ? Colors.blueAccent : Colors.orangeAccent,
                "Kazanma Oranı")),
        const SizedBox(width: 12),
  ...
  Widget _buildStatsGrid(
      double width, double bal, double rpnl, double upnl, double winRate) {
    return GridView.count(
      ...
      children: [
        _buildStatCard("CÜZDAN", "\$${bal.toStringAsFixed(3)}",
            Colors.white, "Net Bakiye"),
        _buildStatCard("REALIZED", "\$${rpnl.toStringAsFixed(3)}",
            rpnl >= 0 ? Colors.greenAccent : Colors.redAccent, "Kapanan K/Z"),
        _buildStatCard("FLOATING", "\$${upnl.toStringAsFixed(3)}",
            upnl >= 0 ? Colors.greenAccent : Colors.redAccent, "Açık İşlemler"),
        _buildStatCard("WIN RATE", "%${winRate.toStringAsFixed(1)}",
            winRate >= 50 ? Colors.blueAccent : Colors.orangeAccent, "Başarı Oranı"),
        _buildKillSwitch(),
      ],
    );
  }
  ...
  Widget _buildStatCard(
      String title, String value, Color valueColor, String subtitle) {
    return Container(
      decoration: BoxDecoration(
    ...
    return InkWell(
      onTap: () {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text("Kill Switch İletildi!",
      ...
      child: Container(
        ...
        child: const Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.power_settings_new, color: Colors.redAccent, size: 26),
            SizedBox(height: 6),
            Text("KILL SWITCH",
                style: TextStyle(
  ...
  Widget _buildChartContainer(
      String title, CustomPainter painter, double currentPnl) {
    return Container(
      width: double.infinity,
  ...
  Widget _buildOpenPositionsPanel(Map<String, double> positions,
      Map<String, double> avgPrices, Map<String, double> marketPrices) {
    List<Widget> rows = [];
    ...
    positions.forEach((symbol, qty) {
      if (qty.abs() > 0.000001) {
        ...
        double pnl = qty > 0
            ? (currentPrice - avgPrices[symbol]!) * qty
            : (avgPrices[symbol]! - currentPrice) * qty.abs();
        double pnlPct = (currentPrice - avgPrices[symbol]!) /
            avgPrices[symbol]! *
            100 *
            (qty > 0 ? 1 : -1);
        ...
        rows.add(Padding(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                  flex: 4,
                  child: Text("\$${avgPrices[symbol]!.toStringAsFixed(2)}",
              ...
              Expanded(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text("${pnl >= 0 ? '+' : ''}\$${pnl.toStringAsFixed(3)}",
                          style: TextStyle(
                              color: pnlColor,
                              fontWeight: FontWeight.bold,
                              fontFamily: 'monospace',
                      ...
                      Text("${pnlPct >= 0 ? '+' : ''}${pnlPct.toStringAsFixed(2)}%",
                          style: TextStyle(
                              color: pnlColor.withOpacity(0.8), fontSize: 10)),
                    ],
                  )),
            ],
          ),
      ...
      child: Column(
        ...
        children: [
          ...
          Expanded(
              child: rows.isEmpty
                  ? const Center(
                      child: Text("Aktif pozisyon bulunmuyor.",
                          style:
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Padding(
            padding: EdgeInsets.all(16.0),
            child: Text("CANLI İŞLEM DEFTERİ",
                style: TextStyle(
                    color: Colors.white54,
                    fontSize: 12,
                    fontWeight: FontWeight.bold,
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final timeStr = DateTime.fromMillisecondsSinceEpoch(
                              r.timestamp.toInt())
                          .toString()
                          .substring(11, isDesktop ? 19 : 16);
                      final symbolClean = r.symbol
                          .replaceAll('usdt', '')
                          .replaceAll('USDT', '')
                          .toUpperCase();
                          ...
                          children: [
                            ...
                            Expanded(
                                ...
                                child: FittedBox(
                                    ...
                                    fit: BoxFit.scaleDown,
                                    child: Text(
                                        "${r.realizedPnl >= 0 ? '+' : ''}${r.realizedPnl.toStringAsFixed(3)}",
                                        textAlign: TextAlign.right,
                                        style: TextStyle(
                                            color: pnlColor,
                                            fontWeight: FontWeight.bold,
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text("Sistem Isınıyor (Blindspot Bekleniyor)",
              style: TextStyle(
                  color: Colors.white,
                  fontSize: 14,
                  fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          const Text("Piyasadan geçmiş vektörler toplanıyor...",
...
class PnLChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < history.length; i++) {
      ...
      if (i == 0) {
        ...
        final prevY = size.height -
            (((history[i - 1] - paddedMin) / paddedRange) * size.height);
    ...
    if (minPnl < 0 && maxPnl > 0) {
      final zeroY =
          size.height - ((0 - paddedMin) / paddedRange) * size.height;
    ...
    final lastY = size.height -
        (((history.last - paddedMin) / paddedRange) * size.height);
    canvas.drawCircle(
        Offset(size.width, lastY), 5, Paint()..color = Colors.white);
    canvas.drawCircle(
        Offset(size.width, lastY), 3, Paint()..color = paint.color);
  }
