      ...
      child: Column(
        children: [
          ...
          _buildZRow(
            ...
            vec.priceVelocity,
            Colors.blueAccent,
            "Fiyat İvmesi (Price Velocity)",
          ),
          _buildZRow(
            "TAHTA (IMB)",
            vec.volumeImbalance,
            Colors.orangeAccent,
            "Emir Defteri Dengesizliği (Orderbook Imbalance)",
          ),
          _buildZRow(
            "DUYGU (NLP)",
            vec.sentimentScore,
            Colors.purpleAccent,
            "Yapay Zeka Haber Duygusu (Neural Sentiment)",
          ),
          _buildZRow(
            "ZİNCİR (URG)",
            vec.chainUrgency,
            Colors.greenAccent,
            "Mempool Ağ Tıkanıklığı (On-Chain Urgency)",
          ),
        ],
      ),
    );
        ...
        child: Row(
          children: [
            ...
            SizedBox(
              width: 50,
              child: Text(
                "${value >= 0 ? '+' : ''}${value.toStringAsFixed(2)}σ",
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text(
                "${currentPnl >= 0 ? '+' : ''}\$${currentPnl.toStringAsFixed(0)}",
                style: TextStyle(
                  color: currentPnl >= 0
                      ? Colors.greenAccent
                      : Colors.redAccent,
          ...
          Expanded(
            child: ClipRect(
              child: CustomPaint(
                painter: PnLChartPainter(balanceHistory, equityHistory),
                size: Size.infinite,
              ),
            ),
          ),
...
class LatencyChartWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    Color latColor = avgLatency > 50
        ? Colors.redAccent
        : (avgLatency > 20 ? Colors.blueAccent : Colors.greenAccent);
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
      ...
      child: Column(
        ...
        children: [
          ...
          Expanded(
            child: ClipRect(
              child: CustomPaint(
                painter: LatencyChartPainter(latencies),
                size: Size.infinite,
              ),
            ),
          ),
...
class PnLChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final minBal = balanceHistory.reduce(min);
    final maxBal = balanceHistory.reduce(max);
    final minEq = equityHistory.reduce(min);
    final maxEq = equityHistory.reduce(max);
    final globalMin = min(minBal, minEq);
    final globalMax = max(maxBal, maxEq);

    final range = (globalMax - globalMin) == 0 ? 1.0 : (globalMax - globalMin);
    final paddedRange = range * 1.3;
    final paddedMin = globalMin - (range * 0.15);

    final dx =
        size.width /
        (balanceHistory.length > 1 ? balanceHistory.length - 1 : 1);
    ...
    for (int i = 0; i < equityHistory.length; i++) {
      ...
      final y =
          size.height -
          (((equityHistory[i] - paddedMin) / paddedRange) * size.height);
      if (i == 0) {
        eqPath.moveTo(x, y);
      } else {
        final prevX = (i - 1) * dx;
        final prevY =
            size.height -
            (((equityHistory[i - 1] - paddedMin) / paddedRange) * size.height);
    ...
    for (int i = 0; i < balanceHistory.length; i++) {
      ...
      final y =
          size.height -
          (((balanceHistory[i] - paddedMin) / paddedRange) * size.height);
      if (i == 0) {
        balPath.moveTo(x, y);
      } else {
        final prevX = (i - 1) * dx;
        final prevY =
            size.height -
            (((balanceHistory[i - 1] - paddedMin) / paddedRange) * size.height);
    ...
    final lastY =
        size.height -
        (((equityHistory.last - paddedMin) / paddedRange) * size.height);
...
class LatencyChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double i = 0; i < size.width; i += 10) {
      canvas.drawLine(Offset(i, slaY), Offset(i + 5, slaY), slaPaint);
    }

    final actualBarWidth = ((size.width / latencies.length) - 2).clamp(
      1.0,
      double.infinity,
    );
    for (int i = 0; i < latencies.length; i++) {
      ...
      Color barColor = lat > 50
          ? Colors.redAccent
          : (lat > 25 ? Colors.blueAccent : Colors.greenAccent);
